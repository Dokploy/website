...
curl --fail --show-error --location https://dokploy.com/install.sh |
  sudo env DOKPLOY_ADVERTISE_ADDR=your-ip bash
```

Pass `DOKPLOY_ADVERTISE_ADDR` inline as shown above. If you export it in your
shell and then run the script with `sudo`, the variable normally will not reach
the installer because `sudo` resets the environment.
...
**For a fresh installation**, pass `DOKPLOY_ENDPOINT_MODE=dnsrr` to the installer.
Systems without the supported kernel capabilities are experimental:
